# U03 — Patente EP3754433A1: leitura comentada das reivindicações e figuras

1. No Balcão, abra o pedido U03 e marque Rodei.
2. Cole o prompt no ChatGPT (GPT-6 Pro).
3. Salve a resposta como U03-entrega.zip em ~/repo/rmup01-digital-twin/_entrada/Balcao/.

---

PEDIDO U03 — Patente EP3754433A1: leitura comentada das reivindicações e figuras
...
TAREFA — Ler a patente EP3754433A1 (e a correspondente US11550262B2 e CH716337A1) no Espacenet/Google Patents e entregar `patente/leitura.md`: (1) o problema que ela declara resolver; (2) reivindicações independentes e dependentes reescritas em linguagem própria, uma a uma; (3) para cada figura, o que mostra (peças, referências numéricas, estados do escape) sem extrair nenhuma medida; (4) o que muda em relação ao escape de alavanca suíça convencional (guard pin, safety roller, dart, chifres do garfo, bancos); (5) o que a patente NÃO diz (dimensões, materiais, contagens); (6) glossário dos termos da patente pt/en/fr. Mais `patente/reivindicacoes.csv` (nº, tipo, resumo, elementos) e `patente/figuras.csv`. Classe A para o que está no texto; D para inferências suas, marcadas.

Devolva um único arquivo zip chamado U03-entrega.zip.

Onde estão os materiais de apoio (texto, pelo conector agenthub-tunnel): list_artifacts(run_id="run_20260921T030232_b5dd42") e get_artifact(artifact_id=..., max_chars=60000). CHARTER.md (convenções de IDs) e ESQUEMAS.md (colunas dos CSV). Se o conector não responder, diga isso em vez de improvisar.
