# U01 — Dossiê oficial Richard Mille: páginas, press kit, especificações

1. No Balcão, abra o pedido U01 e marque Rodei.
2. Cole o prompt no ChatGPT (GPT-6 Pro).
3. Salve a resposta como U01-entrega.zip em ~/repo/rmup01-digital-twin/_entrada/Balcao/.

---

PEDIDO U01 — Dossiê oficial Richard Mille: páginas, press kit, especificações
...
TAREFA — Reunir tudo o que a Richard Mille (e a Ferrari) publicou sobre o RM UP-01: página do modelo, press release, fichas técnicas, PDFs, vídeos oficiais, galerias, comunicados, catálogos, páginas históricas e versões em outros idiomas (en, fr, de, it, ja, zh, es, pt). Para cada âncora do contexto: o trecho literal em que ela aparece, a página e o idioma, ou a marca "não encontrada em fonte primária". Entregar `oficial/dossie.md` (por fonte: o que diz, com trechos curtos), `oficial/ancoras.csv` (âncora, valor, trecho, URL, idioma, classe A ou "não confirmada"), `oficial/midia.csv` (imagens/vídeos oficiais: URL, resolução aparente, o que mostram), fontes.csv, claims.csv.

Devolva um único arquivo zip chamado U01-entrega.zip.

Onde estão os materiais de apoio (texto, pelo conector agenthub-tunnel): list_artifacts(run_id="run_20260921T030231_ee6270") e get_artifact(artifact_id=..., max_chars=60000). CHARTER.md (convenções de IDs) e ESQUEMAS.md (colunas dos CSV). Se o conector não responder, diga isso em vez de improvisar.
