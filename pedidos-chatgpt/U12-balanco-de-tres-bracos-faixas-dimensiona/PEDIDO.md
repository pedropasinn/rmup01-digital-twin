# U12 — Balanço de três braços: faixas dimensionais a partir de 3 mg·cm², 4 Hz e titânio grau 5

1. No Balcão, abra o pedido U12 e marque Rodei.
2. Cole o prompt no ChatGPT (GPT-6 Pro).
3. Salve a resposta como U12-entrega.zip em ~/repo/rmup01-digital-twin/_entrada/Balcao/.

---

PEDIDO U12 — Balanço de três braços: faixas dimensionais a partir de 3 mg·cm², 4 Hz e titânio grau 5
...
TAREFA — Cálculo inverso documentado: dado o momento de inércia publicado de 3 mg·cm², 4 Hz, titânio grau 5 (densidade e módulo com fonte), três braços e seis massas de regulagem, produzir faixas plausíveis de diâmetro do aro, seção do aro, massa total e rigidez da espiral (k = I·(2πf)²), com cada hipótese marcada (E) e o cálculo mostrado (A sobre E). Comparar com balanços de outros ultraplanos (fontes). Entregar `balanco/calculo.md`, `balanco/balanco.py` (stdlib; saída JSON; varredura de hipóteses), `balanco/faixas.csv`, fontes.csv, claims.csv (classes D/E).

Devolva um único arquivo zip chamado U12-entrega.zip.

Onde estão os materiais de apoio (texto, pelo conector agenthub-tunnel): list_artifacts(run_id="run_20260921T030236_d81ef4") e get_artifact(artifact_id=..., max_chars=60000). CHARTER.md (convenções de IDs) e ESQUEMAS.md (colunas dos CSV). Se o conector não responder, diga isso em vez de improvisar.
