# U08 — Imprensa em japonês, chinês, espanhol e português

1. No Balcão, abra o pedido U08 e marque Rodei.
2. Cole o prompt no ChatGPT (GPT-5.6 Pro).
3. Salve a resposta como U08-entrega.zip em ~/repo/rmup01-digital-twin/_entrada/Balcao/.

---

PEDIDO U08 — Imprensa em japonês, chinês, espanhol e português
...
TAREFA — O mesmo levantamento em japonês (Hodinkee Japan, webChronos, Gressive), chinês (Xbiao, WatchsHK, mídia de HK/TW), espanhol (Revista Relojes, Horas y Minutos, Watchtime ES) e português (Watch Time Brasil, Relógios & Canetas). Entregar `imprensa-ja-zh-es-pt/indice.md` (título original + tradução, veículo, data, URL, idioma, o que revela, fotos únicas, confiabilidade), `por-subsistema.md`, fontes.csv, claims.csv.

Devolva um único arquivo zip chamado U08-entrega.zip.

Onde estão os materiais de apoio (texto, pelo conector agenthub-tunnel): list_artifacts(run_id="run_20260921T030234_71c830") e get_artifact(artifact_id=..., max_chars=60000). CHARTER.md (convenções de IDs) e ESQUEMAS.md (colunas dos CSV). Se o conector não responder, diga isso em vez de improvisar.
