# U09 — Entrevistas: Giulio Papi, Richard Mille, Salvador Arbona e engenheiros

1. No Balcão, abra o pedido U09 e marque Rodei.
2. Cole o prompt no ChatGPT (GPT-5.6 Pro).
3. Salve a resposta como U09-entrega.zip em ~/repo/rmup01-digital-twin/_entrada/Balcao/.

---

PEDIDO U09 — Entrevistas: Giulio Papi, Richard Mille, Salvador Arbona e engenheiros
...
TAREFA — Localizar entrevistas, palestras, podcasts e vídeos (com transcrição pública ou legendas) em que Giulio Papi, Richard Mille, Salvador Arbona, Julien Boillat ou outros envolvidos falem do RM UP-01, do calibre RMUP-01, do escape ultraplano, do balanço, da parceria com a Ferrari ou dos desafios de espessura. Entregar `entrevistas/catalogo.csv` (quem, veículo, data, URL, idioma, formato, minuto/trecho relevante), `entrevistas/trechos.md` (citações curtas com atribuição e URL; classe A quando é a pessoa responsável falando), fontes.csv, claims.csv.

Devolva um único arquivo zip chamado U09-entrega.zip.

Onde estão os materiais de apoio (texto, pelo conector agenthub-tunnel): list_artifacts(run_id="run_20260921T030235_1fb82e") e get_artifact(artifact_id=..., max_chars=60000). CHARTER.md (convenções de IDs) e ESQUEMAS.md (colunas dos CSV). Se o conector não responder, diga isso em vez de improvisar.
