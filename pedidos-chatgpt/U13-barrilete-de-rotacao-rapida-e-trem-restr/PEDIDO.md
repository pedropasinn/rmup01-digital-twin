# U13 — Barrilete de rotação rápida e trem: restrições, razões e método do solver

1. No Balcão, abra o pedido U13 e marque Rodei.
2. Cole o prompt no ChatGPT (GPT-6 Pro).
3. Salve a resposta como U13-entrega.zip em ~/repo/rmup01-digital-twin/_entrada/Balcao/.

---

PEDIDO U13 — Barrilete de rotação rápida e trem: restrições, razões e método do solver
...
TAREFA — A partir de ~45 h de reserva, ~6 h por volta do barrilete, 4 Hz e indicação de horas e minutos: (1) deduzir o número de voltas úteis da mola e a razão total barrilete → escape; (2) enumerar arquiteturas de trem compatíveis (número de estágios, faixas de dentes por estágio com módulos plausíveis para um calibre de 1,18 mm, sentido de rotação), como conjunto de candidatos, sem escolher "porque parece bom"; (3) especificar um solver combinatório em Python (stdlib) que recebe centros medidos, diâmetros observados e as restrições, e devolve candidatos com resíduos; entregar o solver funcionando com dados de exemplo marcados como exemplo. Entregar `trem/restricoes.md`, `trem/candidatos.csv`, `trem/gear_search.py`, fontes.csv, claims.csv (D/E).

Devolva um único arquivo zip chamado U13-entrega.zip.

Onde estão os materiais de apoio (texto, pelo conector agenthub-tunnel): list_artifacts(run_id="run_20260921T030237_86c9ea") e get_artifact(artifact_id=..., max_chars=60000). CHARTER.md (convenções de IDs) e ESQUEMAS.md (colunas dos CSV). Se o conector não responder, diga isso em vez de improvisar.
