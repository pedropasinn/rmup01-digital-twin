# U06 — Imprensa técnica em inglês: resumos por subsistema

1. No Balcão, abra o pedido U06 e marque Rodei.
2. Cole o prompt no ChatGPT (GPT-5.6 Pro).
3. Salve a resposta como U06-entrega.zip em ~/repo/rmup01-digital-twin/_entrada/Balcao/.

---

PEDIDO U06 — Imprensa técnica em inglês: resumos por subsistema
...
TAREFA — Catalogar os artigos em inglês com conteúdo técnico ou fotos de primeira mão sobre o RM UP-01 (Hodinkee, SJX/Watches by SJX, Monochrome, Revolution, WatchTime, Europa Star, FT/HTSI, Fratello, Quill & Pad, TimeZone, A Blog to Watch, etc.). Para cada um: o que revela por subsistema (caixa, cristais, platina/pontes, barrilete, corda, seletor W/H, trem, escape, balanço/espiral, indicação, rubis, choque, fixações), quais fotos únicas contém (descrição, não o arquivo), qualidade e confiabilidade justificadas, paywall sim/não. Entregar `imprensa-en/indice.md`, `imprensa-en/por-subsistema.md` (o melhor material por subsistema), fontes.csv, claims.csv (só B/D com fonte).

Devolva um único arquivo zip chamado U06-entrega.zip.

Onde estão os materiais de apoio (texto, pelo conector agenthub-tunnel): list_artifacts(run_id="run_20260921T030233_b006b9") e get_artifact(artifact_id=..., max_chars=60000). CHARTER.md (convenções de IDs) e ESQUEMAS.md (colunas dos CSV). Se o conector não responder, diga isso em vez de improvisar.
