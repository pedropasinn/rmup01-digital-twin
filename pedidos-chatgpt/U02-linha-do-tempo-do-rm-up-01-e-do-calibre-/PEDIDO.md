# U02 — Linha do tempo do RM UP-01 e do calibre RMUP-01

1. No Balcão, abra o pedido U02 e marque Rodei.
2. Cole o prompt no ChatGPT (GPT-5.6 Pro).
3. Salve a resposta como U02-entrega.zip em ~/repo/rmup01-digital-twin/_entrada/Balcao/.

---

PEDIDO U02 — Linha do tempo do RM UP-01 e do calibre RMUP-01
...
TAREFA — Reconstruir a cronologia pública: anúncio, lançamento, edições, tiragem, preço de lançamento, prêmios, aparições (feiras, leilões), evolução da comunicação (o que a marca disse em cada momento), o programa de desenvolvimento (6 000 h, protótipos, parceria Audemars Piguet Le Locle/APRP) e o que a Ferrari publicou. Entregar `cronologia/linha-do-tempo.md` (data, evento, fonte, classe), `cronologia/pessoas.csv` (quem falou o quê: Richard Mille, Giulio Papi, Salvador Arbona, engenheiros, jornalistas), fontes.csv, claims.csv.

Devolva um único arquivo zip chamado U02-entrega.zip.

Onde estão os materiais de apoio (texto, pelo conector agenthub-tunnel): list_artifacts(run_id="run_20260921T030231_a0c7c9") e get_artifact(artifact_id=..., max_chars=60000). CHARTER.md (convenções de IDs) e ESQUEMAS.md (colunas dos CSV). Se o conector não responder, diga isso em vez de improvisar.
