# U07 — Imprensa em francês, alemão e italiano

1. No Balcão, abra o pedido U07 e marque Rodei.
2. Cole o prompt no ChatGPT (GPT-5.6 Pro).
3. Salve a resposta como U07-entrega.zip em ~/repo/rmup01-digital-twin/_entrada/Balcao/.

---

PEDIDO U07 — Imprensa em francês, alemão e italiano
...
TAREFA — O mesmo levantamento do pedido de imprensa em inglês, agora em francês (Le Temps, Montres Passion, Watchonista, Journal Suisse d'Horlogerie, GMT Magazine), alemão (Uhren-Kosmos, Watchtime.net, Chronos, NZZ) e italiano (Orologi & Passioni, Il Giornale degli Orologi, Chrono24 IT). Entregar `imprensa-fr-de-it/indice.md` (título original + tradução do título, veículo, data, URL, idioma, o que revela por subsistema, fotos únicas, confiabilidade), `imprensa-fr-de-it/por-subsistema.md`, fontes.csv, claims.csv.

Devolva um único arquivo zip chamado U07-entrega.zip.

Onde estão os materiais de apoio (texto, pelo conector agenthub-tunnel): list_artifacts(run_id="run_20260921T030234_426d5e") e get_artifact(artifact_id=..., max_chars=60000). CHARTER.md (convenções de IDs) e ESQUEMAS.md (colunas dos CSV). Se o conector não responder, diga isso em vez de improvisar.
